...
class  Youtube extends Video
{
    ...
    public function downloadVideo($url=null)
    {
        ...
        $filePath = parent::VIDEOTMPDIR.$fileName.'.'.$quality['container'];
        $itag = $quality['itag'];
        exec("you-get --no-caption  --itag=".$itag." -o ". parent::VIDEOTMPDIR." -O $fileName '{$this->grab_address}' ",$info);
        ...
        if(!file_exists($filePath)){
            $cmd = "you-get --no-caption  --itag=".$itag." -o ". parent::VIDEOTMPDIR." -O $fileName '{$this->grab_address}' ";
            return  ['ret'=>false,'msg' => $this->grab_address.'--下载失败'.'--'.$cmd.'--'.var_export($info,true)];
        }
        $fileName.='.'.$quality['container'];
   ...
   public  function downloadImages($url)
   {
       ...
       $filePath = parent::VIDEOTMPDIR.$fileName;
       $i = 1;
